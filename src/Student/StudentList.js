...
const StudentList = () => {
  ...
  const fetchGradesAndSections = useCallback(async () => {
    try {
      ...
      const gradeResponse = await axios.get('https://spring-csdrms.onrender.com/class/allUniqueGrades');
      setGrades(gradeResponse.data);

      // Fetch sections for the selected grade
      if (selectedGrade) {
        const sectionResponse = await axios.get(`https://spring-csdrms.onrender.com/class/sections/${selectedGrade}`);
  ...
  const handleDeleteStudent = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://spring-csdrms.onrender.com/student/delete/${id}/${loggedInUser.userId}`);
