...
const AddLogBookModal = ({ isOpen, onClose, refreshRecords }) => {
  ...
  const fetchRecords = async () => {
    try {
      const response = await axios.get('https://spring-csdrms.onrender.com/record/getRecordsByStudentDetails', {
        params: {
  ...
  const fetchGrades = async () => {
    try {
      const response = await axios.get('https://spring-csdrms.onrender.com/class/allUniqueGrades', {
        headers: { Authorization: `Bearer ${authToken}` },
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post('https://spring-csdrms.onrender.com/record/insertMultipleRecords', payload, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
